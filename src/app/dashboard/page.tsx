...
export default async function DashboardPage() {
    ...
    // Busca o usuário no banco para exibir a role (perfil) e o nome
        ...
        <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-8">
            <div className="bg-gradient-to-tr from-indigo-600 to-purple-700 rounded-3xl p-8 sm:p-12 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="mt-6 inline-flex items-center bg-white/20 px-4 py-2 rounded-xl backdrop-blur-sm border border-white/10 shadow-sm relative z-10">
                    ...
                    <span className="font-bold text-sm tracking-widest uppercase">Perfil: {dbUser?.role}</span>
                </div>
            </div>

            {/* Painel Exclusivo: CGPG / Admin (Visão Macro) */}
                    ...
                    <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6">
                            {turmasProgress.map(turma => (
                                <div key={turma.id} className="group">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="font-bold text-slate-700 flex items-center gap-2">
                                            {turma.ano_serie} {turma.nome}
                                        </span>
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-8 text-center shadow-sm max-w-2xl mx-auto">
                    ...
                    <p className="text-slate-500 mb-6">
                        {(dbUser?.role === 'Professor')
                            ? 'Acesse a aba "Meus Guias" no menu lateral para importar novos Guias via IA e avaliar os Apontamentos de suas turmas.'
                            : 'Acesse a aba "Meus Guias" no menu lateral para visualizar os conteúdos semanais e, caso seja líder, enviar seus relatórios de turma.'}
                    </p>
